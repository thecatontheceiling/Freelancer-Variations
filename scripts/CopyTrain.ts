...
export const afterDeploy: ModScript["afterDeploy"] = async function (context, api) {
  ...
  if(fs.existsSync(`${context.config.runtimePath}/chunk7patch4.rpkg`)) fs.copyFileSync(context.config.runtimePath + '/chunk7patch4.rpkg', context.config.runtimePath + '/chunk8patch204.rpkg');
  ...
  if(fs.existsSync(`${context.config.runtimePath}/chunk2patch4.rpkg`)) fs.copyFileSync(context.config.runtimePath + '/chunk2patch4.rpkg', context.config.runtimePath + '/chunk8patch209.rpkg');
